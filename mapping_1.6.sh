...
blastn \
	...
	-outfmt "6 qseqid sseqid qlen slen sstrand pident length mismatch gapopen qstart qend sstart send"
...
if [ -n "$sort_size" ]; then
	BS=1
	BE=$(grep -c '' 01_blast/$BLAST_RESULTS)
	...
	echo -e "$SR" > 01_blast/$BLAST_RESULTS_SORT
	echo $BE
else
...
BS=1
BE=$(grep -c '' 01_blast/$BLAST_RESULTS_SORT)
